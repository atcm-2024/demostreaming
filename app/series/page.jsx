...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const fetchMovies = async () => {
      try {
        ...
        /** filtado de Api por año 2010 y por series */
       setMovies(response.data.entries.filter(item => item.releaseYear >= 2010 && item.programType === 'series') ); 
      } catch (error) {
  ...
  return (
    <div className="h-5/6 w-full grid md:grid-cols-4 grid-cols-2 lg:grid-cols-7 gap-6 justify-center items-center overflow-auto">
      {movies.length > 0 ? (
        movies.map((mov, i) => (
          <div key={i} className="flex flex-row ml-1 w-5/6 mb-7">
            <figure className="flex flex-col h-[170px]">
